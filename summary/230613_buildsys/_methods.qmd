...
We used a seperate method in order to compare the efficacy of the smoothing technique that we developed. We chose to use an SVM, as it has been shown in litereature (de Rautlin de Roy) to have robust performance across a wide range of features. SVMs are straightforward to implement with relatively few hyperparameters, as compared to other methods that have been recently shown to have high performce on the window detection problem. [^6]

*How this method works*

SVMs extend simple linear[^7] regressions. In a similar way to a linear regression, SVMs approximate a line of best fit using the features and labels provided [^8]. However, with the use of kernel functions that can map high dimensionanal feature spaces to ones of lower dimensions, SVMs also effectively create hyperplanes of best fit  in order to classify datasets. The use of a kernel function also enables the introduction of mappings that may be a better fit for a given data set.

*How we implemented it* 

Our focus was trying to get the best performance from the SVM using an optimal data set with optimal pre-processing functions applied[^9]. We therefore came up with combinations of the various features we had access to, as well as their derivatives and differences from one another. The base features were: ambient temperature, measured temperature, the differnece between ambient tempaerature and measured temperature, the difference between measured temperature and the derivative of measured temperature. We also had the same features for telative humidity. After creating combinations of these base features, we had a test set of 113[^10] combinations as shown in @fig-feature-combos. 
