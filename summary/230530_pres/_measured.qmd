...
## 3. difference and derivatives 
```{python}


fig = w1.plot_analysis()

for name in ["Observed Temp", "Smoothed", "Deriv1", "Deriv2"]:
    fig.update_traces(visible="legendonly",selector=dict(name=name))
fig.show()
...
for name in ["Observed Temp","Smoothed", "Difference",  "Deriv2"]:
    fig.update_traces(visible="legendonly",selector=dict(name=name))
fig.show()
...
for name in ["Observed Temp","Smoothed", "Difference", "Deriv"]:
    fig.update_traces(visible="legendonly",selector=dict(name=name))
fig.show()
